export const useRohermetImagenesDestacadas = () => {
    ...
    const fetchImagenesDestacadas = async () => {
        ...
        try {
            ...
            const imagenesWithUrls = (data || []).map(imagen => ({
                ...imagen,
                imagen_chica: imagen.imagen_chica ? getRohermetImagenDestacadaUrl(imagen.imagen_chica) : null,
                imagen_mediana: imagen.imagen_mediana ? getRohermetImagenDestacadaUrl(imagen.imagen_mediana) : null,
                imagen_grande: imagen.imagen_grande ? getRohermetImagenDestacadaUrl(imagen.imagen_grande) : null
            }))
    ...
    const fetchImagenDestacadaById = async (id) => {
        ...
        try {
            ...
            const imagenWithUrls = {
                ...data,
                imagen_chica: data.imagen_chica ? getRohermetImagenDestacadaUrl(data.imagen_chica) : null,
                imagen_mediana: data.imagen_mediana ? getRohermetImagenDestacadaUrl(data.imagen_mediana) : null,
                imagen_grande: data.imagen_grande ? getRohermetImagenDestacadaUrl(data.imagen_grande) : null
            }
    ...
    const createImagenDestacada = async (imagenData, imagenChica, imagenMediana, imagenGrande) => {
        ...
        try {
            ...
            if (imagenChica) {
                imagenChicaPath = await uploadRohermetImagenDestacadaChica(imagenChica, imagenType)
            }

            if (imagenMediana) {
                imagenMedianaPath = await uploadRohermetImagenDestacadaMediana(imagenMediana, imagenType)
            }

            if (imagenGrande) {
                imagenGrandePath = await uploadRohermetImagenDestacadaGrande(imagenGrande, imagenType)
            }
        ...
        try {
            ...
            if (imagenChica) {
                if (currentData?.imagen_chica) {
                    await deleteRohermetImagenDestacada(currentData.imagen_chica)
                }
                imagenChicaPath = await uploadRohermetImagenDestacadaChica(imagenChica, imagenType)
            }
            ...
            if (imagenMediana) {
                if (currentData?.imagen_mediana) {
                    await deleteRohermetImagenDestacada(currentData.imagen_mediana)
                }
                imagenMedianaPath = await uploadRohermetImagenDestacadaMediana(imagenMediana, imagenType)
            }
            ...
            if (imagenGrande) {
                if (currentData?.imagen_grande) {
                    await deleteRohermetImagenDestacada(currentData.imagen_grande)
                }
                imagenGrandePath = await uploadRohermetImagenDestacadaGrande(imagenGrande, imagenType)
            }
    ...
    const deleteImagenDestacadaCompleta = async (id) => {
        ...
        try {
            ...
            if (imagen?.imagen_chica) {
                await deleteRohermetImagenDestacada(imagen.imagen_chica)
            }
            if (imagen?.imagen_mediana) {
                await deleteRohermetImagenDestacada(imagen.imagen_mediana)
            }
            if (imagen?.imagen_grande) {
                await deleteRohermetImagenDestacada(imagen.imagen_grande)
            }
