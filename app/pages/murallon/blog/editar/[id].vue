<template>
    <DefaultSection>
        <HeadingH1>Editar Blog Murallón</HeadingH1>
        <div v-if="loading" class="flex justify-center items-center py-12">
            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
        </div>
        <MurallonBlogForm v-else :is-editing="true" :initial-data="currentBlog" @submit="handleSubmit"
            @cancel="handleCancel" />
    </DefaultSection>
</template>
...
const { currentBlog, loading, fetchBlogById, updateBlog } = useMurallonBlog()
