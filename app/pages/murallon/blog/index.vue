<template>
    <DefaultSection>
        <HeadingH1>Blog Murallón</HeadingH1>
        <ButtonPrimary :to="ROUTE_NAMES.MURALLON.CREAR_BLOG">
            Crear Nuevo Blog
        </ButtonPrimary>
        ...
        <TableLayout v-else :data="blogs" :columns="tabla.columns" :empty-state-text="`No hay blogs creados`"
            table-name="blogs" @edit="handleEdit" @delete="handleDelete" />
    </DefaultSection>
</template>
...
const { blogs, loading, fetchBlogs, deleteBlog } = useMurallonBlog()
...
const handleEdit = (blog) => {
    navigateTo(ROUTE_NAMES.MURALLON.EDITAR_BLOG(blog.id))
}
