<template>
    <DefaultSection>
        <HeadingH1>Editar Producto Murallón</HeadingH1>
        ...
        <div v-if="!loading && !error" class="w-full max-w-md lg:max-w-[56.25rem]">
            <MurallonProductoForm ref="productoFormRef" :is-editing="true" :initial-data="currentProducto"
                :show-buttons="false" @submit="handleFormSubmit" @cancel="handleCancel" />
        </div>

        <div v-if="!loading && !error"
            class="w-full max-w-md lg:max-w-[56.25rem] flex flex-col lg:flex-row items-center gap-5 mt-8">
...
const { currentProducto, loading, error, fetchProductoById, updateProducto } = useMurallonProductos()
...
const handleFormSubmit = async (formData) => {
    ...
    try {
        ...
        await updateProducto(id, formData.productoData, formData.imagen, formData.fichaTecnica, formData.removedFiles, formData.galeria, formData.removedImages)
